...
# --- 0a. ephemeral local secrets --------------------------------------------
# QA D-108: this drill must run from a CLEAN CLONE with zero setup. It used to
# depend on an untracked alerts-api/.dev.vars that only existed on the machine
# that wrote it (and in CI, which minted its own) — so the AC §G4 proof was not
# reproducible. The drill now generates its own throwaway secrets, and the
# Worker refuses to boot without them, so a fail-open run is impossible too.
# Nothing here is a real credential: they live for the length of this script.
step "0a. generate an ephemeral alerts-api/.dev.vars (no real secret exists)"
rand_secret() { head -c 32 /dev/urandom | od -An -tx1 | tr -d ' \n'; }
ADMIN_SECRET="$(rand_secret)"
TOKEN_SECRET="$(rand_secret)"
DEV_VARS="$REPO_ROOT/alerts-api/.dev.vars"
if [ -e "$DEV_VARS" ]; then
  cp "$DEV_VARS" "$DEV_VARS.drill-backup"
  echo "existing .dev.vars set aside as .dev.vars.drill-backup"
fi
cat > "$DEV_VARS" <<VARS
# Generated by scripts/alert_drill.sh — ephemeral, throwaway, never committed
# (.dev.vars is gitignored). Regenerated on every run.
ADMIN_SECRET = "$ADMIN_SECRET"
TOKEN_SECRET = "$TOKEN_SECRET"
MAIL_API_URL = "$CATCHER/email"
MAIL_API_TOKEN = "local-catcher-no-account"
VARS
restore_dev_vars() {
  rm -f "$DEV_VARS"
  [ -e "$DEV_VARS.drill-backup" ] && mv "$DEV_VARS.drill-backup" "$DEV_VARS"
  return 0
}
cleanup() {
  for g in "${pgids[@]:-}"; do kill -- "-$g" 2>/dev/null || true; done
  restore_dev_vars
}
echo "wrote $DEV_VARS with fresh 64-hex-char secrets"

# --- 0b. the fail-closed proof ----------------------------------------------
# Before the real run, prove the Worker REFUSES to serve with an empty
# TOKEN_SECRET rather than minting tokens under a zero-length HMAC key.
step "0b. fail-closed proof: an empty TOKEN_SECRET must stop the Worker dead"
cd "$REPO_ROOT/alerts-api"
cat > .dev.vars.failopen <<VARS
ADMIN_SECRET = "$ADMIN_SECRET"
TOKEN_SECRET = ""
MAIL_API_URL = "$CATCHER/email"
MAIL_API_TOKEN = "local-catcher-no-account"
VARS
cp "$DEV_VARS" "$DEV_VARS.good"
cp .dev.vars.failopen "$DEV_VARS"
setsid env -u CLOUDFLARE_API_TOKEN -u CLOUDFLARE_ACCOUNT_ID \
  npx wrangler dev --local --port 8787 > "$EVIDENCE/worker-failopen.log" 2>&1 &
FAILOPEN_PGID=$!
for _ in $(seq 1 60); do curl -s -o /dev/null "$API/api/health" && break; sleep 1; done
CODE=$(curl -s -o "$EVIDENCE/00-failclosed-response.json" -w '%{http_code}' "$API/api/health")
kill -- "-$FAILOPEN_PGID" 2>/dev/null || true
rm -f .dev.vars.failopen
cp "$DEV_VARS.good" "$DEV_VARS"; rm -f "$DEV_VARS.good"
sleep 2
cat "$EVIDENCE/00-failclosed-response.json"; echo
[ "$CODE" = "503" ] || { echo "FAIL: Worker served HTTP $CODE with an empty TOKEN_SECRET — it must refuse (503)"; exit 1; }
grep -q 'misconfigured' "$EVIDENCE/00-failclosed-response.json" || { echo "FAIL: no misconfiguration detail returned"; exit 1; }
echo "OK: empty TOKEN_SECRET => 503 misconfigured, no route served, no token minted"
